...
import ffmpeg
import os
os.environ["IMAGEIO_FFMPEG_EXE"] = "/Users/siyuanyang/Documents/AI/中科院/venv/ffmpeg"
from moviepy.editor import VideoFileClip


img = cv2.imread("newImageFile.png")
plt.figure()
plt.imshow(img[:, :, ::-1])
plt.show()
nx = 6
ny = 4
file_paths = glob.glob("/Users/siyuanyang/Documents/AI/中科院/testcases1/*")
# img = cv2.imread("realroad.jpeg")
# plt.figure()
# plt.imshow(img[:, :, ::-1])
# plt.title("raw")
# plt.show()
...
# 相机校正：外参，内参，畸变系数(get the parameters of the camera)
def cal_calibrate_params(file_paths):
    ...
    # 相机校正 (reset the view of camera)
    ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(object_points, image_points, gray.shape[::-1], None, None)
    print("yes")
    return ret, mtx, dist, rvecs, tvecs


# 图像去畸变：利用相机校正的内参，畸变系数(make the img away from the undistorted)
def img_undistort(img, mtx, dist):
    dis = cv2.undistort(img, mtx, dist, None, mtx)
    return dis


# 车道线提取(get the pipline)
...
def cal_line_param(binary_warped):
    ...
    margin = 100
    # 设置最小像素点，阈值用于统计滑动窗口区域内的非零像素个数，小于30的窗口不对x的中心值进行更新
    minpix = 50
    # 用来记录搜索窗口中非零点在nonzeroy和nonzerox中的索引
    left_lane_inds = []
    right_lane_inds = []

    # 遍历该副图像中的每一个窗口(solve all windows in the img)
...
def cal_radius(img, left_fit, right_fit):
    ...
    right_curverad = ((1 + (2 * right_fit_cr[0] * right_y_axis * ym_per_pix * right_fit_cr[1]) ** 2) ** 1.5) / np.absolute(
        (2 * right_fit_cr[0]))
...
if __name__ == "__main__":
    ...
    points = [[350, 238], [434, 238],
              [243, 319], [582, 319]]
    # img = cv2.line(img, , (683, 448), (0, 0, 255), 3)
    ...
    # points = [[774, 266], [1941, 266], [268, 811], [2450, 811]]
    img = cv2.line(img, lb, rb, (0, 0, 250), 3)
    img = cv2.line(img, lb, lt, (0, 0, 250), 3)
    img = cv2.line(img, rt, lt, (0, 0, 250), 3)
    img = cv2.line(img, rb, rt, (0, 0, 250), 3)
...
def process_image(img):
    ...
    # left_fit, right_fit = cal_line_param(rigin_pipline_img)
    # 绘制安全区域
    result = fill_lane_poly(transform_img, left_fit, right_fit)
    # result = fill_lane_poly(rigin_pipline_img, left_fit, right_fit)
...
# 视频处理
clip1 = VideoFileClip("hh.mp4")
white_clip = clip1.fl_image(process_image)
# white_clip.ipython_display()
white_clip.write_videofile("outputfreal02.mp4", audio=False)
